...
def draw_data(data):
    ...
    plt.imshow(data.detach().numpy(), interpolation='nearest', origin='lower')
    plt.colorbar()
    plt.show()
class DepthwiseConv1D(nn.Module):
    def __init__(self, in_channels, kernel_size):
        ...
        self.bn3 = nn.BatchNorm1d(in_channels)  # Batch Normalization layer
    def forward(self, x):
        skip = x
        ...
        out = self.bn2(out)
        out = out + skip/2
        out = self.bn3(out)  # Applying Batch Normalization
        return out
...
conv_block = Net_test( in_channels=390,
                d_model=256,
                kernel_size=17,
                num_layers=1,
                num_heads=4,
...
# checkpoint = torch.load('/content/drive/MyDrive/VIetNamese_sign_language/output/weights/cfg_3/_13/checkpoint_last_seed-1.pth')
# model_state_dict = checkpoint['model']
# conv_block.load_state_dict(model_state_dict)
# output = conv_block(inputs)
